...
4.Annotated Tag: Persistent text pointer
to specific commit

Ex. Let's suppose we have two files in
working directory file1.txt and file2.txt
After initializing git, adding both files
to staging area and finally commiting
them as 'initial commit'
Internally git will create one commit
object containing description of commit,
author name and author email and 
associate a SHA1 hash with that.
Also note that Commit is just a wrapper 
around tree object.
Therefore it will point to a tree object
which will eventually contain links to
two blobs-file1.txt and file2.txt
...
To commit the files from staging area and
...
Untracked->Staged: git add[for new files]
Staged->Unmodified: git commit
After commiting if you change file in the
working directory,
It will change state Unmodified->Modified
Again you've to use git add to add it to
staging area and do further commit.
Files can also be untracked, means can be
moved from Unmodified->Untracked
...
to unmodified.
Now the file is present in all the three 
areas i.e. in working directory, in 
staging area as well as in git repository
After changing the content of the file,
it changes to the modified state in the
working directory, next we have to add it
to staging area and finally commit that.
...
Pointers for all branches are located in
.git/refs/heads
Current branch tracks new commits.
Branch pointer moves automatically after
every new commit
...
directory and then looking up the content
of the mentioned path.
e.g. cat refs/heads/master
It should point to the last commit if you
have not checked out to any other commit.

If we checkout to some other commit using
commit SHA1hash Id, then head will be 
detached from the branch as branch is
pointing to the last commit.
To point head back to the master branch,
...
uses:
1.Fast Forward Merge: It is possible when
there are no further commits in the 
receiving branch after the commit where
new branch was created.
For this, git simply shift the receiving
branch pointer and the head pointer to
the pointer where new branch is pointing
i.e. at the very last commit in the new
branch. There will not be any commits in 
this case while merging.
